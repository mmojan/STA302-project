...
```{r}
original_sales<-read.csv("VideoGamesSales-3.csv")
...
sales <- data.frame(read.csv("VideoGamesSales-3.csv"))
...
```{r}
library(ggplot2)
ggplot(VideoGamesSales_3, aes(x = Critic_Score + User_Score, y = log(Global_Sales))) +
  geom_point() + 
...
```
Above is a graph of our sample data, comparing the percent change of sales with respect to the combination of user and critic score. As we can see there appears to be a positive correlation between our predictor variables and the change in sales. This is a preliminary model which we built on and corrected in our methods section to provide greater analysis to our research question. 
